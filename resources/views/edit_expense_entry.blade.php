                        ...
                        <div class="card-body">
							...
							<form class="row g-2" method="POST" action="{{route('update_expence_entry')}}">
                                    ...
                                    <div class="yearWrapper">
                                        ...
                                        <select class="multiple-select" data-placeholder="Choose anything"
                                            ...
                                            @foreach ($year as $medicals)
                                                <option value="{{ $medicals->id }}" @if ($ven->select_year == $medicals->id)
													selected
												@endif>
                                    ...
                                    <select class="multiple-select" data-placeholder="Choose anything" id="month"
                                        ...
                                        @foreach (range(1, 12) as $monthNumber)
                                            ...
                                            <option value="{{ $monthName }}"
                                                @if ($ven->select_month == $monthName) selected @endif>
                                                {{ $monthName }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="multiple-select" data-placeholder="Choose anything" name="select_company"
                                        ...
                                        @foreach ($company as $medicals)
                                            <option value="{{ $medicals->id }}" @if ($ven->select_company == $medicals->id)
												selected
											@endif>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="multiple-select" data-placeholder="Choose anything" id="head" name="expence_head">
                                        <option value="">Select</option>
                                        <option value="Expences" @if ($ven->expence_head == 'Expences')
											selected @endif>Expences</option>
                                        <option value="Vendor" @if ($ven->expence_head == 'Vendor')
											selected @endif>Vendor</option>
                                        <option value="Star" @if ($ven->expence_head == 'Star')
											selected @endif>Star</option>
                                        ...
                                        <div class="col-md-6 " style="float-left">
                                            <table class="items_table table table-bordered width80" id="table">
                                                ...
                                                <tbody class="add_more">
													@foreach ($expense_entry_list as $expense_entry_lists)
													<tr><td><input type="hidden"  name="expence_head[]" value="{{$expense_entry_lists->expence_head}}"><input type="hidden"  name="select_category[]" required="" style="border:none; width: 100%;" value = "{{$expense_entry_lists->select_category}}"><input type="text"  name="expense[]" required="" style="border:none; width: 100%;" value="{{$expense_entry_lists->select_expense}}"></td><td><input type="text"  name="amount[]" required="" style="border:none; width: 100%;" value="{{$expense_entry_lists->amount}}"></td><td><a href="{{route('delete_added_expence_entry',$expense_entry_lists->id)}}"><button type="button" class="btn1 btn-outline-danger delete-row"><i class="bx bx-trash me-0"></i></button></a></td></tr>
                                    ...
                                    @foreach ($exp_entry as $exp_entrys)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a href="{{route('edit_expence_entry',$exp_entrys->expense_entries_id)}}">
												<button type="button" class="btn1 btn-outline-success"><i class='bx bx-edit-alt me-0'></i></button> </a>
                                                <a
                                                    href="{{ route('destroy_expence_entry', $exp_entrys->expense_entry_id) }}">
                    ...
                    success: function(result) {
                        ...
                        $.each(recordss, function(a, b)

                            {
                                ...
                                $("#records").append('<tr><td>' + b.category +
                                    '</td><td>' + b.select_expense + '</td><td>' + b.amount + '</td></tr>');
