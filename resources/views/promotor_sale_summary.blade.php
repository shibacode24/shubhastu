@php
    ...
    $groupedData = collect($data)
        ->groupBy(function ($item) {
            return $item['select_stokist_id'] .
                '|' .
                $item['select_medical_id'] .
                '|' .
                $item['select_marketing_id'] .
                '|' .
                $item['sale_of_month'];
        })
        ->map(function ($group) {
            ...
            $medicineArray = $group
                ->groupBy(function ($item) {
                    return $item['ptrs'] . '|' . $item['mpss'] . '|' . $item['select_batchs'];
                })
...
<div class="row g-2">
    ...
    @if (auth()->check())
        ...
        <div class="col-md-2">
            @php
                $count =
                    ((float) $proreport[0]->total_grand_total1) -
                    ((float) $proreport[0]->tds / 100) * (float) $proreport[0]->total_grand_total1;
            ...
            <tbody>
                ...
                @if (auth()->guard('marketings')->check())
                    @php
                        ...
                        $authUser = auth()->guard('marketings')->user();
        ...
        <div class="row g-2">
            ...
            @for ($i = 0; $i < count($groupedData); $i++)
                ...
                @if (empty(auth()->guard('marketings')->user()->id) ||
                        auth()->guard('marketings')->user()->id == $groupedData[$i]['select_marketing_id']
                )
                    <div class="col-md-2">
                    ...
                    @php
                        ...
                        //     'select_stokist_id' => $groupedData[$i]['select_stokist_id'], 'select_medical_id' => $groupedData[$i]['select_medical_id']]);
                        // // ->groupby('append_no');
                        ...
                        $query22 = DB::table('promotorsalemedicine')
                            ->join(
                                ...
                                'promotor__sales.id',
                                '=',
                                'promotorsalemedicine.promotor__sales_id',
                            )
                            ->where([
                                        ...
                                        $query = DB::table('promotorsalemedicine')
                                            ...
                                            ->where([
                                                ...
                                                'promotor__sales.select_marketing_id' =>
                                                    $groupedData[$i]['select_marketing_id'],
                                            ]);
