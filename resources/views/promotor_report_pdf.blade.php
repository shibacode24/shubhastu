...
<h3>Medicine Sale Report</h3>
<!-- Year, Month, Company, Marketing, and Client Details -->
<table class="bordered-table" style="width:100%">
    <tr>
        <td><b>Year</b></td>
        <td>{{ $proreport[0]->year }}</td>
    </tr>
    <tr>
        <td><b>Month</b></td>
        <td>{{ $proreport[0]->sale_of_month }}</td>
    </tr>
    <tr>
        <td><b>Company</b></td>
        <td>{{ $proreport[0]->Name }}</td>
    </tr>
    <tr>
        <td><b>Marketing</b></td>
        <td>{{ $proreport[0]->name }}</td>
    ...
    <tr>
        <td><b>Client</b></td>
        <td>{{ $proreport[0]->allotted_dr_name }}</td>
    </tr>
    <tr>
        <td><b>MPS Total</b></td>
        <td>{{ $proreport[0]->total_grand_total1 }}</td>
    </tr>
    <tr>
        <td><b>PTR Total</b></td>
        <td>{{ $proreport[0]->total_grand_total2 }}</td>
    </tr>
    <tr>
        @php
        ...
        @endphp
        <td><b>TDS</b></td>
        <td>{{ number_format($tds_value, 2) }}</td>
    </tr>
    <tr>
        @php
...
<table class="bordered-table" style="width:100%">
    <thead>
        <tr>
            <th>Medicine</th>
            <th>Quantity</th>
            <th>Quantity X MPS(Total)</th>
            <th>Quantity X PTR(Total)</th>
        </tr>
    </thead>
    <tbody>
        @php
...
<!-- Grouped Data -->
  @for ($i = 0; $i < count($groupedData); $i++)
    ...
    <table class="bordered-table" style="width:100%">
        <tr>
            <td><b>Stockist:</b></td>
            <td>{{ $groupedData[$i]['select_stokist_id'] }}</td>
        </tr>
        <tr>
            <td><b>Medical:</b></td>
            <td>{{ $groupedData[$i]['select_medical_id'] }}</td>
        </tr>
        <tr>
            <td><b>Marketing:</b></td>
            <td>{{ $groupedData[$i]['name'] }}</td>
        </tr>
        @php
        $grandtot1 = 0;
        $grandtot2 = 0;
        ...
        $query22 = DB::table('promotorsalemedicine')
            ...
            ->where([
                ...
                'promotor__sales.year_id' => $groupedData[$i]['year_id'],
                                'promotor__sales.select_marketing_id' => $groupedData[$i]['select_marketing_id'],
            ])
    ...
    @endphp
        <tr>
            <td><b>Grand Total 1:</b></td>
            <td>{{ number_format($grandtot1, 2) }}</td>
        </tr>
        <tr>
            <td><b>Grand Total 2:</b></td>
            <td>{{ number_format($grandtot2, 2) }}</td>
        </tr>
    </table>
    <br>
    {{-- <hr> --}}
    <table class="bordered-table" style="width:100%">
        <thead>
            <tr>
                ...
                <th>MPS</th>
                <th>Quantity</th>
                <th>Qnty_mps_total</th>
                <th>Qnty_ptr_total</th>
            </tr>
        </thead>
        <tbody>
